...
from utils.llm_util import call_openai_sdk_large, call_openai_sdk_small
...
model_large_config = global_config['model_large']
model_small_config = global_config['model_small']
...
def sql_gen_and_execute(db_env: DataBaseEnv, query: str):
    ...
    param = {"model": model_large_config['name'], "messages": messages,"key":model_large_config['key'],"url":model_large_config['url']}

    try:
        response = call_openai_sdk_large(**param)
        ...
        # return sql_res['truncated_results']
        return markdown_res
...
def sql_fix(dialect: str, mschema: str, query: str, sql_query: str, error_info: str):
    ...
    # logging.info(f"【错误信息】: {error_info}")

    messages = [
        {"role": "system", "content": system_prompt},
        {"role": "user", "content": user_prompt}
    ]
    param = {"model": model_small_config['name'], "messages": messages,"key":model_small_config['key'],'url':model_small_config['url']}
    
    response = call_openai_sdk_small(**param)
    content = response.choices[0].message.content
    sql_query = extract_sql_from_qwen(content)
